...
public abstract class LimitedCombineFileInputFormat<K, V>
    extends FileInputFormat<K, V> {

  private static final Log LOG = LogFactory.getLog(LimitedCombineFileInputFormat.class);
  ...
  public static final String SPLIT_MAXBLOCKS =
      "mapreduce.input.fileinputformat.split.maxblocks";

  public static final int SPLIT_MAXBLOCKS_DEFAULT = 2000;
   ...
   * default constructor
   */
  public LimitedCombineFileInputFormat() {
  }
  ...
  public List<InputSplit> getSplits(JobContext job)
    ...
    int maxNumBlocks = conf.getInt(SPLIT_MAXBLOCKS, SPLIT_MAXBLOCKS_DEFAULT);
    ...
    LOG.info("Combining files with properties:");
    LOG.info("\tcount:  "+stats.size());
    LOG.info("\tlength: "+totalLen);
    LOG.info("Using limits: ");
    LOG.info("\tmaxSize: "+maxSize);
    LOG.info("\tminSizeNode: "+minSizeNode);
    LOG.info("\tminSizeRack: "+minSizeRack);
    LOG.info("\tmaxNumBlocks: "+maxNumBlocks);
    ...
    for (MultiPathFilter onepool : pools) {
      ...
      getMoreSplits(job, myPaths, maxSize, minSizeNode, minSizeRack, maxNumBlocks, splits);
    }

    // create splits for all files that are not in any pool.
    getMoreSplits(job, stats, maxSize, minSizeNode, minSizeRack, maxNumBlocks, splits);
  ...
  private void getMoreSplits(JobContext job, List<FileStatus> stats,
                             long maxSize, long minSizeNode, long minSizeRack, int maxNumBlocks,
    ...
    while (true) {
      ...
      for (Iterator<Map.Entry<String, Set<OneBlockInfo>>> iter = nodeToBlocks
        ...
        while (oneBlockIter.hasNext()) {
          ...
          // create this split.
          if ((maxSize != 0 && curSplitSize >= maxSize) || validBlocks.size() >= maxNumBlocks) {
    ...
    while (blockToNodes.size() > 0) {
      ...
      for (Iterator<Map.Entry<String, List<OneBlockInfo>>> iter =
        ...
        if (!validBlocks.isEmpty()) {
          if ((minSizeRack != 0 && curSplitSize >= minSizeRack) || validBlocks.size() >= maxNumBlocks) {
